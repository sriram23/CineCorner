    ...
    return (
        <div className='flex flex-col items-center m-4 p-4 w-52 border-secondary bg-secondary  border rounded-md ' style={{maxWidth: '50rem', width: '100%'}}>
            <figure className='w-40 h-40 m-0'>
                {data.profile_path ? <img className='object-cover rounded-full w-full h-full' src={`https://image.tmdb.org/t/p/w500/${data.profile_path}`} alt={"Picture of "+data.name} /> : <img className='rounded-full object-cover w-40 h-40' src={AVATAR} alt={"Picture of "+data.name+" - unavailable"} />}
            </figure>
